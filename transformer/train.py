import torch
import torch.nn.functional as F
from torch.utils.data import DataLoader
import matplotlib.pyplot as plt
import time
import sys
import os
import math

sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from char_lm.dataset import CharDataset
from transformer import GPTModel

# ── config ────────────────────────────────────────────────
DATA_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
    "data", "input.txt"
)

BLOCK_SIZE   = 128
BATCH_SIZE   = 32
D_MODEL      = 128
N_HEADS      = 4
N_LAYERS     = 6
MAX_STEPS    = 10000
EVAL_EVERY   = 100
LR           = 3e-4
WARMUP_STEPS = 500
MIN_LR       = 3e-5
VAL_SPLIT    = 0.1
DEVICE       = "cuda" if torch.cuda.is_available() else "cpu"
# ──────────────────────────────────────────────────────────

print(f"Device: {DEVICE}")

# ── data ──────────────────────────────────────────────────
full_dataset = CharDataset(DATA_PATH, BLOCK_SIZE)
vocab_size   = full_dataset.vocab_size

n       = len(full_dataset)
n_val   = int(n * VAL_SPLIT)
n_train = n - n_val

train_dataset, val_dataset = torch.utils.data.random_split(
    full_dataset, [n_train, n_val]
)

train_loader = DataLoader(
    train_dataset,
    batch_size=BATCH_SIZE,
    shuffle=True,
    pin_memory=(DEVICE == "cuda"),
)

val_loader = DataLoader(
    val_dataset,
    batch_size=BATCH_SIZE,
    shuffle=False,
    pin_memory=(DEVICE == "cuda"),
)

print(f"Vocab size:     {vocab_size}")
print(f"Train samples:  {n_train:,}")
print(f"Val samples:    {n_val:,}")

# ── helpers ───────────────────────────────────────────────
def infinite_loader(loader):
    while True:
        for batch in loader:
            yield batch


def get_lr(step):
    # phase 1: linear warmup
    if step < WARMUP_STEPS:
        return LR * step / WARMUP_STEPS
    # phase 2: cosine decay
    progress = (step - WARMUP_STEPS) / (MAX_STEPS - WARMUP_STEPS)
    cosine   = 0.5 * (1 + math.cos(math.pi * progress))
    return MIN_LR + (LR - MIN_LR) * cosine


@torch.no_grad()
def estimate_val_loss(model, val_loader, num_batches=20):
    model.eval()
    losses = []
    for i, (x, y) in enumerate(val_loader):
        if i >= num_batches:
            break
        x, y = x.to(DEVICE), y.to(DEVICE)
        _, loss = model(x, y)
        losses.append(loss.item())
    model.train()
    return sum(losses) / len(losses)


@torch.no_grad()
def generate_from_prompt(model, dataset, prompt,
                         max_new_tokens=200, temperature=0.8):
    model.eval()

    ids = dataset.encode(prompt)
    prompt_len = len(ids)

    if len(ids) < BLOCK_SIZE:
        pad_id = ids[0]                              # ← use first token of prompt
        ids = [pad_id] * (BLOCK_SIZE - len(ids)) + ids

    ids = ids[-BLOCK_SIZE:]
    idx = torch.tensor([ids], dtype=torch.long, device=DEVICE)

    for _ in range(max_new_tokens):
        idx_cond = idx[:, -BLOCK_SIZE:]
        logits, _ = model(idx_cond)
        logits = logits[:, -1, :] / temperature
        probs  = F.softmax(logits, dim=-1)
        next_id = torch.multinomial(probs, num_samples=1)
        idx = torch.cat([idx, next_id], dim=1)

    model.train()

    # decode only from where prompt starts
    generated_ids = idx[0][BLOCK_SIZE - prompt_len:].tolist()
    return dataset.decode(generated_ids)


# ── train ─────────────────────────────────────────────────
def train(model, name):
    model = model.to(DEVICE)
    optimizer = torch.optim.AdamW(model.parameters(), lr=LR)

    train_losses = []
    val_losses   = []
    lr_history   = []   # NEW: track LR over time
    steps        = []

    print(f"\n{'-'*40}")
    print(f"Model:  {name}")
    print(f"Params: {model.num_parameters():,}")
    print(f"{'-'*40}")

    gen   = infinite_loader(train_loader)
    start = time.time()

    for step in range(1, MAX_STEPS + 1):

        # ── NEW: apply LR schedule every step ──
        lr = get_lr(step)
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr

        x, y = next(gen)
        x, y = x.to(DEVICE), y.to(DEVICE)

        logits, loss = model(x, y)

        optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        optimizer.step()

        if step % EVAL_EVERY == 0:
            val_loss = estimate_val_loss(model, val_loader)
            elapsed  = time.time() - start

            train_losses.append(loss.item())
            val_losses.append(val_loss)
            lr_history.append(lr)   # NEW
            steps.append(step)

            print(
                f"  step {step:>5} | "
                f"train {loss.item():.4f} | "
                f"val {val_loss:.4f} | "
                f"lr {lr:.2e} | "
                f"{elapsed:.1f}s"
            )

    # generate from prompts
    print(f"\n-- Samples from {name} --")
    for prompt in ["ROMEO:", "KING RICHARD:", "First Citizen:"]:
        print(f"\nPrompt: '{prompt}'")
        print(generate_from_prompt(model, full_dataset, prompt))

    return steps, train_losses, val_losses, lr_history


# ── run ───────────────────────────────────────────────────
model = GPTModel(
    vocab_size = vocab_size,
    block_size = BLOCK_SIZE,
    d_model    = D_MODEL,
    n_heads    = N_HEADS,
    n_layers   = N_LAYERS,
)

steps, train_losses, val_losses, lr_history = train(model, "GPT-6L-128D-SAN")

# save model
torch.save(model.state_dict(), "gpt_6l_128d.pt")
print("\nModel saved to gpt_6l_128d.pt")

# ── plot ──────────────────────────────────────────────────
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8))

# top plot — loss curves
ax1.plot(steps, train_losses, label="Train Loss", color="darkorange", linewidth=2)
ax1.plot(steps, val_losses,   label="Val Loss",   color="steelblue",  linewidth=2)
ax1.set_ylabel("Loss")
ax1.set_title("GPT Transformer — Train vs Val Loss")
ax1.legend()
ax1.grid(True, alpha=0.3)

# bottom plot — LR schedule
ax2.plot(steps, lr_history, label="Learning Rate", color="green", linewidth=2)
ax2.set_ylabel("Learning Rate")
ax2.set_xlabel("Step")
ax2.set_title("Learning Rate Schedule (Warmup + Cosine Decay)")
ax2.legend()
ax2.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig("transformer_train_val.png", dpi=150)
plt.show()
print("\nPlot saved to transformer_train_val.png")


